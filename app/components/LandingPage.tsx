...
function ServiceCard({ service, index, isActive }: { service: typeof SERVICES[number]; index: number; isActive: boolean }) {
  ...
  const fanAngle = (index - 1.5) * 4
  const fanY = Math.abs(index - 1.5) * 12

  return (
    <motion.div
      ref={cardRef}
      className="cursor-pointer"
      initial={{ opacity: 0, y: 60, rotate: fanAngle }}
      animate={isActive ? { opacity: 1, y: 0, rotate: fanAngle } : {}}
    ...
    >
      <motion.div
        ...
        style={{
          background: `linear-gradient(135deg, ${service.color}12 0%, ${service.color}05 100%)`,
      ...
      >
        ...
        <div
          ...
          style={{ background: `${service.color}18`, color: service.color, transform: 'translateZ(30px)' }}
        >
          <span className="text-lg font-bold">{service.name.charAt(0)}</span>
        </div>
        <h3 className="text-xl font-semibold mb-2" style={{ color: NAVY, transform: 'translateZ(20px)' }}>
          {service.name}
        </h3>
        <p className="text-sm leading-relaxed" style={{ color: NEARBLACK, opacity: 0.5, transform: 'translateZ(10px)' }}>
          {service.description}
        </p>
        <div
...
function ServicesSlide({ isActive }: { isActive: boolean }) {
  return (
    <section className="relative h-screen w-full snap-start snap-always flex flex-col justify-center items-center px-6 md:px-10 overflow-hidden" style={{ background: OFFWHITE }}>
      <motion.h2
        className="text-3xl md:text-5xl font-bold tracking-tight mb-12 md:mb-16"
      ...
      <div className="flex items-center justify-center gap-4 md:gap-6 flex-wrap md:flex-nowrap">
        {SERVICES.map((service, i) => (
          <ServiceCard key={service.name} service={service} index={i} isActive={isActive} />
        ))}
      </div>
    </section>
  )
  ...
  return (
    <section className="relative h-screen w-full snap-start snap-always flex items-center px-6 md:px-10 overflow-hidden" style={{ background: NAVY }}>
      ...
      <div className="w-full max-w-6xl mx-auto relative z-10 flex flex-col md:flex-row items-center gap-12 md:gap-20">
            ...
            >
              <div className="flex items-center gap-4 mb-2">
                {typeof collab.logo === 'string' ? (
                  /* eslint-disable-next-line @next/next/no-img-element */
                  <img src={collab.logo} alt={collab.name} className="h-7 w-auto brightness-0 invert opacity-80" />
                ) : (
                  <Image src={collab.logo} alt={collab.name} className="h-7 w-auto brightness-0 invert opacity-80" />
                )}
              </div>
              <p
                ...
                style={{
                  color: OFFWHITE,
                  opacity: active === i ? 0.85 : 0.4,
                }}
              >
                {collab.description}
              </p>
              <div
                className="flex flex-wrap gap-2 mt-3 overflow-hidden transition-all duration-500"
                style={{
              ...
              >
                {collab.techs.map((tech) => (
                  <span
                    ...
                    style={{ background: 'rgba(241,242,241,0.08)', color: OFFWHITE, opacity: 0.7 }}
                  >
                    {tech}
                  </span>
                ))}
        ...
        <div className="md:w-1/2">
          <motion.h2
            className="text-4xl md:text-6xl lg:text-7xl font-bold leading-[1.05] tracking-tight text-right"
            style={{ fontFamily: "'Poppins', sans-serif", color: OFFWHITE }}
            initial={{ opacity: 0, clipPath: 'inset(100% 0 0 0)' }}
            animate={isActive ? { opacity: 1, clipPath: 'inset(0 0 0 0)' } : {}}
            transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
          ...
          <form className="flex flex-col gap-4" onSubmit={handleSubmit} noValidate>
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-1">
                <input type="text" placeholder="Nombre" value={nombre} onChange={e => { setNombre(e.target.value); if (errors.nombre) setErrors(prev => ({ ...prev, nombre: undefined })) }} className="flex-1 px-4 py-3 text-sm rounded-lg outline-none transition-all duration-300 focus:ring-2 focus:ring-white/20" style={{ background: 'rgba(241,242,241,0.08)', color: OFFWHITE, border: errors.nombre ? '1px solid #ef4444' : '1px solid rgba(241,242,241,0.12)' }} />
                {errors.nombre && <span className="text-xs" style={{ color: '#ef4444' }}>{errors.nombre}</span>}
              </div>
              <div className="flex flex-col gap-1">
                <input type="email" placeholder="Email" value={email} onChange={e => { setEmail(e.target.value); if (errors.email) setErrors(prev => ({ ...prev, email: undefined })) }} className="flex-1 px-4 py-3 text-sm rounded-lg outline-none transition-all duration-300 focus:ring-2 focus:ring-white/20" style={{ background: 'rgba(241,242,241,0.08)', color: OFFWHITE, border: errors.email ? '1px solid #ef4444' : '1px solid rgba(241,242,241,0.12)' }} />
                {errors.email && <span className="text-xs" style={{ color: '#ef4444' }}>{errors.email}</span>}
              </div>
            </div>
            <div className="flex flex-col gap-1">
              <textarea placeholder="Cuéntanos sobre tu proyecto" rows={4} value={mensaje} onChange={e => { setMensaje(e.target.value); if (errors.mensaje) setErrors(prev => ({ ...prev, mensaje: undefined })) }} className="px-4 py-3 text-sm rounded-lg outline-none resize-none transition-all duration-300 focus:ring-2 focus:ring-white/20" style={{ background: 'rgba(241,242,241,0.08)', color: OFFWHITE, border: errors.mensaje ? '1px solid #ef4444' : '1px solid rgba(241,242,241,0.12)' }} />
              {errors.mensaje && <span className="text-xs" style={{ color: '#ef4444' }}>{errors.mensaje}</span>}
            </div>
            <button type="submit" className="px-8 py-3.5 text-sm font-semibold rounded-lg transition-all duration-300 hover:scale-105 active:scale-95 self-start" style={{ background: OFFWHITE, color: NAVY }}>
              Enviar
            </button>
          </form>
        </motion.div>
